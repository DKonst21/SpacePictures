...
def fetch_epic_pictures_of_the_day(api_key_parameter):
    ...
    epic_pictures = get_list_epic_pictures_params(api_key_parameter)
    ...
    for index_number, picture_info in enumerate(epic_pictures):
        ...
        name_epic_picture_template = os.path.join(get_default_catalog(), "{name}.png".format(name=image_name))
        download_images(actual_foto_url, name_epic_picture_template, api_key_parameter)
